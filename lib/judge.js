...
2. Score the BRIDGE first - how well the answer connects the two cards - then add bonuses.
   ...
   BASE
   1-3  no bridge at all (1 = empty, gibberish, offensive, or the noun card repeated back word for word; 2 = "a bunch of / a lot of / a group of"; 3 = an existing real collective noun, or a random word, with no twist)
...
A full phrase "a/an ___ of <noun>". It must bridge BOTH cards: the wordplay has to be about the NOUN and land in the SCENARIO. Before you commit, run the swap test - mentally replace the noun card with an unrelated one. If the phrase still works ("a deduction of geologists" works just as well for accountants or detectives), it is anchored only to the scenario: bin it and find one that could only be said about THIS noun. Aim your entry at the band given as GOOSE TARGET below - on a lower target play a decent but unspectacular answer rather than your best - and do not reuse the player's key word or structure. It must also LAND WITHOUT SPECIALIST KNOWLEDGE: if getting it needs insider jargon, regional trivia or a notation only enthusiasts know ("a B-9 of sirens" needs US bingo-card lettering plus a homophone leap), it is too clever - pick something a family at a kitchen table would get. Give bot_note: at most 12 words saying plainly how your answer works. The same gettable-without-a-footnote rule applies to the alternatives. Whatever you end up writing, score it honestly with the same rubric as bot_score; if a player out-punned you, their score must be higher than yours.

EDGE CASES
NEVER accuse the player of something they did not do. "Repeated the noun" means the answer IS the noun card word for word - "Sirens" on a Sirens card. Anything else is a real attempt: judge it on merit. Do not call an answer lazy, or a repeat, or the wrong part of speech, unless it actually is - check the words in front of you before you say it. If you cannot find the joke, say so plainly rather than inventing a fault the player did not commit.
Empty, whitespace, gibberish, or the noun repeated word for word: score 1, badge lazy (chaos if gibberish), and make the deflection the joke ("The card said Dragons and you said 'asdf'. Bold. Wrong, but bold.").
...
HOW TO READ AN ANSWER: the player types into the blank in "a ___ of ${noun}". A bare word IS that blank - read "shout" as "a shout of ${noun}" and judge the whole phrase, not the fragment. If they already wrote a full phrase, judge it as written. A one-word answer is a normal answer, never lazy by itself.

PLAYER ANSWER: ${JSON.stringify(String(answer ?? '').slice(0, 200))}

Respond with exactly this JSON shape:
{"hits":["pun","sound","scenario","image"],"score":1,"verdict":"","comment":"","badge":"","bot_answer":"","bot_score":1,"bot_note":"","alternatives":["",""],"image_idea":""}
...
- bot_note: at most 12 words, plainly how your bot_answer works. If you cannot explain it in 12 plain words, it is too obscure - choose a different answer.
...
Respond with exactly this JSON shape:
{"results":[{"name":"","hits":[],"score":1,"verdict":"","comment":"","badge":""}],"winner":"","bot_answer":"","bot_score":1,"bot_note":"","alternatives":["",""],"image_idea":""}
...
// Like str, but never severs a word: the Goose's note is prose, and "...a real collective
// noun for ca" reads as a bug rather than a cap.
const clip = (v, max) => {
  const t = typeof v === 'string' ? clean(v, max * 4) : '';
...
function normalise(parsed, input, meta = {}) {
  ...
  const bot = {
    ...
    note: clip(parsed.bot_note, 130), // shown under the Goose's entry so it shows its working
  };
...
async function mockJudge(input, options = {}) {
  ...
  const top = results.reduce((w, r) => (r.score > w.score ? r : w), results[0]);
  const bot = {
    answer: `a mock-ery of ${noun.toLowerCase()}`,
    score: 6 + (hash32(cards.join('|')) % 3),
    note: 'Mockery, because this judge is a stand-in.',
  };
