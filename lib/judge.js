...
2. Hits set the BAND; pick the exact score inside it by quality:
   0 hits -> 1-3   (1 = empty, gibberish, offensive, or just the noun repeated; 2 = "a bunch of / a lot of / a group of"; 3 = an existing real collective noun or a random word with no twist)
   1 hit  -> 4-5   plain but fitting
   2 hits -> 6-7   a decent pun or a decent scene
   3 hits -> 8-9   clever AND fits the scenario
   4 hits -> 9-10  10 is reserved for the genuinely brilliant: layered, tight, and people will repeat it later. Expect to give a 10 less than one time in twenty.
   ...
   1 "penguins" | 2 "a bunch of penguins" | 3 "a colony of penguins" | 4 "a sad of penguins" | 5 "a mourning of penguins" | 6 "a tuxedo of penguins" (great image, noun only, ignores the funeral) | 7 "a waddle of mourners" | 8 "a black-tie of penguins" | 9 "a pallbearing of penguins" | 10 "a wake of penguins" (funeral wake + the wake behind a swimming bird + a real collective noun for vultures).
...
A full phrase "a/an ___ of <noun>". It must bridge BOTH cards: the wordplay has to be about the NOUN and land in the SCENARIO. Before you commit, run the swap test - mentally replace the noun card with an unrelated one. If the phrase still works ("a deduction of geologists" works just as well for accountants or detectives), it is anchored only to the scenario: bin it and find one that could only be said about THIS noun. Aim for the 8-9 band, and do not reuse the player's key word or structure. Score it honestly with the same rubric as bot_score; if a player out-punned you, their score must be higher than yours.
 ...
 */
function buildSinglePrompt(cards, answer, { spicy = false } = {}) {
  const noun = cards[0] || '?';
...
{"hits":["pun","sound","scenario","image"],"score":1,"verdict":"","comment":"","badge":"","bot_answer":"","bot_score":1,"image_idea":""}
 ...
 * @param {{spicy?: boolean}} opts
 */
function buildPartyPrompt(cards, players, { spicy = false } = {}) {
  const noun = cards[0] || '?';
  const scenario = cards[1] || '';
  const list = players
...
Respond with exactly this JSON shape:
{"results":[{"name":"","hits":[],"score":1,"verdict":"","comment":"","badge":""}],"winner":"","bot_answer":"","bot_score":1,"image_idea":""}
...
function buildPrompts({ cards, answers, spicy = false }) {
  const party = answers.length >= 2;
  return {
    mode: party ? 'party' : 'single',
    system: SYSTEM_PROMPT,
    user: party
      ? buildPartyPrompt(cards, answers, { spicy })
      : buildSinglePrompt(cards, answers[0] ? answers[0].answer : '', { spicy }),
...
function validateRequest(body) {
  ...
  return { cards, answers, spicy: body.spicy === true };
}
